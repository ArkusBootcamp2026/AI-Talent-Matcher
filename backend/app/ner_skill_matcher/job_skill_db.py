"""Job skill database operations using CSV files"""
...
def load_all_job_skill_dbs() -> pd.DataFrame:
    """
    Load and merge all *_job_roles_skills.csv files in data/db/.
    Always reads fresh from disk - no caching.
    """
    csv_files = sorted(settings.CSV_DB_DIR.glob("*_job_roles_skills.csv"))
    ...
    if not dfs:
        raise ValueError(f"No *_job_roles_skills.csv files found in {settings.CSV_DB_DIR}")
